...
import { GATE_LOOP_LENGTH, KITE_HOME_Y, KITE_LIMIT_X, KITE_LIMIT_Y, NITRO_GATE_BONUS, NITRO_MAX } from './kite-circuit/course';
import KiteCircuitScene from './kite-circuit/Scene';
import type { GatePose, GateState, NitroState, RunState } from './kite-circuit/types';
...
export default function KiteCircuit() {
  ...
  const nitro = useRef<NitroState>({ amount: 0, held: false, active: false, intensity: 0 });
  ...
  const resetRun = useCallback(() => {
    ...
    nitro.current = { amount: 0, held: false, active: false, intensity: 0 };
    setScore(0);
    setCombo(0);
    setNitroAmount(0);
  ...
  const resolveGate = useCallback((gate: GateState, pose: GatePose) => {
    ...
    if (distance < pose.passRadius) {
      comboRef.current += 1;
      scoreRef.current += 100 + comboRef.current * 25;
      nitro.current.amount = Math.min(NITRO_MAX, nitro.current.amount + NITRO_GATE_BONUS);
      gate.cycle += 1;
      setCombo(comboRef.current);
      setScore(scoreRef.current);
      setNitroAmount(nitro.current.amount);
      return;
    }
  ...
  useEffect(() => {
    ...
    const updateSteering = (now: number) => {
      ...
      if (runStateRef.current === 'running' && pressedKeys.current.size > 0) {
        ...
        const steerSpeed = 3.35;
        target.current.x = Math.max(-KITE_LIMIT_X, Math.min(KITE_LIMIT_X, target.current.x + (horizontal / length) * steerSpeed * delta));
        target.current.y = Math.max(KITE_HOME_Y - KITE_LIMIT_Y, Math.min(KITE_HOME_Y + KITE_LIMIT_Y, target.current.y + (vertical / length) * steerSpeed * delta));
      }
  ...
  return (
    ...
    >
      <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0.46, 9.2], fov: 42 }} gl={{ antialias: true }}>
        <KiteCircuitScene runId={runId} runState={runStateRef} scoreRef={scoreRef} flightDistance={flightDistance} player={player} target={target} nitro={nitro} onNitroChange={setNitroAmount} reducedMotion={reducedMotion} onResolve={resolveGate} />
      </Canvas>
      ...
      {runState === 'running' && (
        <button
          ...
          onPointerUp={() => { nitro.current.held = false; }}
          onPointerLeave={() => { nitro.current.held = false; }}
          onPointerCancel={() => { nitro.current.held = false; }}
        >
          BOOST
        </button>
      )}
          ...
          <div className="max-w-xl">
            {runState === 'intro' && (
              <>
                ...
                <p className="mx-auto mt-6 max-w-md text-base leading-relaxed text-white/75 sm:text-lg">Guide the paper plane through the monument frames. Miss the flight line and the run is over.</p>
